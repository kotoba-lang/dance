(ns kotoba.dance
  "VRM dance stage as data — the CLJ/EDN twin of `kami-live` (ADR-0043/0044/0045),
   projected onto the render-IR `kotoba.webgpu` already consumes.
   ...
   which `kotoba.webgpu/draw!` (or the WebGL2 path) renders with no GPU-specific code.
...
(defn frame-ir
  "Project the whole dance stage at time `t` (seconds) to a render-IR map for
   `kotoba.webgpu/draw!`. Pure + deterministic. The performer(s), stage set, crowd and
  ...
  (let [grid (beat-at scene t)
        ...
        ;; rig: :vrm (articulated VRM humanoid, default) or :blocks (the 6-box figure).
        rig (get-in scene [:dance/avatar :rig] :vrm)
        people (mapcat
                 (fn [m]
                   (let [move (or (:dance m)
                     ...
                     (if (= rig :blocks)
                       ...
                       (vrm/dance-avatar pose (:home m) (:skin m) (:beat-frac grid)))))
                 members)
...
;; is the kotoba.netsync schema for `:stage/presence`; transport (WebRTC/kami-rtc) is the
...
(defn valid-frame?
  "A cheap structural check on a projected frame (mirrors kotoba.webgpu.ir/valid?)."
  [ir]
